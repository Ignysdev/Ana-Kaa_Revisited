# Añã-Kaa
Projeto originalmente desenvolvido no IFBA para a disciplina de oficina III, com o objetivo de estimular o debate acerca da invasão de terras indígenas, a preservação da identidade de um povo e o impacto das ações humanas na natureza. Agora atualizado para versões mais recentes e portado para computadores. 

Este foi um projeto originalmente desenvolvido pelo seguinte grupo:

Programadores: Victor Silva (Email: visipasa@gmail.com) e Bruno Simões (Email: ignysdev@gmail.com).

Musicista: Victor Silva

Artista: Rodrigo Souza (instagram: @rodrgoos)

Roteirista e gestor de projeto: Anselmo (https://github.com/Akashic002)

Artista auxiliar: Victor Hugo (instagram: @vituruguu)

Revisitação: Bruno Simões (Email: ignysdev@gmail.com) e Victor Hugo (instagram: @vituruguu).
